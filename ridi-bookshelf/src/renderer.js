...
function renderCategories() {
  ...
  elements.categoryList.innerHTML = state.shelf.categories
    .map((category) => `
  ...
  elements.categoryFilter.innerHTML = [
    ...
    ...state.shelf.categories.map((category) => {
...
function filteredBooks() {
  ...
  return booksArray().filter((book) => {
    ...
    const categoryMatch = activeAuthor || state.activeCategoryId === "all" || book.categoryId === state.activeCategoryId;
    const periodMatch = activeAuthor || !book.period || book.period === state.activePeriod || placements.some((placement) => placement.period === state.activePeriod);
...
function renderSummary(filtered) {
  ...
  elements.summary.innerHTML = `
    <div class="dashboard-card">
      ...
      <div class="stat">
        <span>\uC218\uC9D1 \uAE30\uAC04</span>
        <strong class="blue-stat">${escapeHtml(periodNameForId(state.activePeriod))}</strong>
      </div>
      <div class="stat">
        <span>\uC77D\uB294 \uC911 / \uC644\uB3C5</span>
